...
import numpy as np
import time
def GetLiquidLevelCurve(Image,VesselMask,MaxViewAngleDeg=10,Mode="Canny",NormScore=False,MinScore=4,MinRelativeScore =0.7,MaxCurves=4,LineThikness=2,MinWidth=0.25,IgnoreTopBottum=True):
    ...
    # Gray1 = (np.power(Gray/255.0, 1.5) * 255.0).astype(np.uint8)
    # Gray2 = (np.power(Gray/255.0, 0.5) * 255.0).astype(np.uint8)
