...
class ATSSAssigner(nn.Module):
    ...
    def forward(self,
        ...
        distances, ac_points = dist_calculator(gt_bboxes.reshape([-1, 4]), anc_bboxes)  #caculate distance between gt and anchor centor
        ...
        mask_pos = is_pos * is_in_gts * mask_gt  #>thres,in_gt_mask,batch_mask

        target_gt_idx, fg_mask, mask_pos = select_highest_overlaps(
            mask_pos, overlaps, self.n_max_boxes)   #获得select_anchor对应的gt_box,同一个anchor属于多个gt时选取iou大的
        #target_gt_idx:anchors belongs to which gt ,fg_mask: which anchor has gt, mask_pos: anchor belong to which gt for each gt
    ...
    def select_topk_candidates(self,
        ...
        for per_level_distances, per_level_boxes in zip(level_distances, n_level_bboxes):
            ...
            _, per_level_topk_idxs = per_level_distances.topk(selected_k, dim=-1, largest=False)   #选取中心最近的topk个anchor
            ...
            is_in_candidate = torch.where(is_in_candidate > 1,
                torch.zeros_like(is_in_candidate), is_in_candidate)  #防错误超限
            is_in_candidate_list.append(is_in_candidate.to(distances.dtype))  #dim(batch,max_num_gt,n_level_bboxes) one-hot nearest anchor
            start_idx = end_idx

        is_in_candidate_list = torch.cat(is_in_candidate_list, dim=-1)
        candidate_idxs = torch.cat(candidate_idxs, dim=-1)

        return is_in_candidate_list, candidate_idxs  #candidate_idxs id in all_num_bboxes
    ...
    def thres_calculator(self,
        ...
        _candidate_overlaps = torch.where(is_in_candidate > 0,
            overlaps, torch.zeros_like(overlaps))  #is_in_candidate one-hot replace 1 with overlap
        ...
        overlaps_mean_per_gt = candidate_overlaps.mean(axis=-1, keepdim=True)  #只是选topk个anchor,不必关心是否in gt, mean,std不考虑存在空值.
    ...
    def get_targets(self,
        ...
        #target_gt_idx means the idx for anchor belong to which gt-box, turn in into flatten type for each batch add batch_idx * self.n_max_boxes, use fg_mask to control with  target_gt_idx = 0 whether belongs to box
        ...
        target_scores = target_scores[:, :, :self.num_classes]  #对full_like self.bg_idx 无目标的anchor进行类别截断
